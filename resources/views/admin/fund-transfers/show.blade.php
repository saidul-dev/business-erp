@php
    $amountInWords = \App\Support\AmountInWords::convert((float) $transfer->amount, \App\Models\CompanySetting::current()->currency_label);
@endphp
<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Transfer detail') }}</p>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
        ...
        <div class="order-2 lg:order-1 space-y-6">
                ...
                <div class="p-6">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <div class="flex items-center gap-3 text-sm">
                            <div>
                                <span class="block text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('From') }}</span>
                                <span class="block font-semibold text-slate-800">{{ $transfer->fromAccount->name }}</span>
                            </div>
                            <svg class="h-5 w-5 text-slate-300" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/></svg>
                            <div>
                                <span class="block text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('To') }}</span>
                                <span class="block font-semibold text-slate-800">{{ $transfer->toAccount->name }}</span>
                            </div>
                        </div>
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700 ring-1 ring-violet-200">
                    ...
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-5 text-sm">
                        <div>
                            ...
                            <span class="block text-slate-700">{{ $transfer->transfer_date->format('d M, Y') }}</span>
                        </div>
                        <div>
                            <span class="block text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Recorded By') }}</span>
                            <span class="block text-slate-700">{{ $transfer->creator->name ?? '—' }}</span>
                        </div>
                        ...
                        <div>
                            ...
                            <span class="block text-slate-700">{{ $transfer->reference_no }}</span>
                        </div>
                        @endif
                    </div>
            ...
            @if ($transfer->ledgerTransaction)
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <h3 class="font-bold text-brand-900">{{ __('Ledger Entry') }}</h3>
                    <span class="text-xs font-semibold text-slate-400">{{ $transfer->ledgerTransaction->voucher_no }}</span>
                </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($transfer->ledgerTransaction->lines as $line)
                        <tr>
        ...
        <div class="order-1 lg:order-2 lg:sticky lg:top-6 space-y-4">
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="p-5">
                    <span class="text-[11px] font-semibold uppercase tracking-wide text-violet-700">{{ __('Amount') }}</span>
                    <div class="mt-1 font-mono text-3xl font-black tabular-nums text-violet-700">{{ number_format($transfer->amount, 2) }}</div>
                    <p class="mt-3 border-t border-slate-100 pt-3 text-xs italic text-slate-500">{{ $amountInWords }}</p>
                </div>
            </div>

            <div class="flex flex-col gap-2">
                <a href="{{ route('fund-transfers.print', $transfer) }}" target="_blank"
                   class="inline-flex items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-700">
