<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('General Settings') }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Business profile shown on invoices, reports and print templates') }}</p>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ route('settings.update') }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <div class="mb-4">
                    <h3 class="font-bold text-brand-900">{{ __('Business Logo') }}</h3>
            ...
            <div class="lg:col-span-2 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 space-y-5">
                <div>
                    <h3 class="font-bold text-brand-900">{{ __('Business Details') }}</h3>
                    <p class="text-xs text-slate-400">{{ __('Name and contact information') }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <x-input-label for="legal_name" :value="__('Legal / Registered Name')" />
        ...
        <div class="mt-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="mb-4">
                <h3 class="font-bold text-brand-900">{{ __('Finance & Compliance') }}</h3>
                <p class="text-xs text-slate-400">{{ __('Used across accounts, reports and Mushak invoice formatting') }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    ...
                    <select id="currency" name="currency" required
                        ...
                        @foreach (['BDT' => __('BDT — Taka'), 'USD' => __('USD — US Dollar'), 'EUR' => __('EUR — Euro')] as $code => $label)
                ...
                <div>
                    <x-input-label for="financial_year_start_month" :value="__('Financial Year Starts')" />
                    <select id="financial_year_start_month" name="financial_year_start_month" required
                            class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-accent-500 focus:ring-accent-500">
                        @foreach ([__('January'), __('February'), __('March'), __('April'), __('May'), __('June'), __('July'), __('August'), __('September'), __('October'), __('November'), __('December')] as $i => $monthName)
                ...
                <div>
                    <x-input-label for="vat_registration_no" :value="__('VAT / Mushak Reg. No.')" />
        ...
        <div class="mt-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            <div class="mb-4">
                ...
                <p class="text-xs text-slate-400">{{ __('Controls the public company website that will run alongside this admin panel') }}</p>
            </div>
            ...
            <label class="flex items-start gap-4 rounded-xl bg-slate-50 px-5 py-4 cursor-pointer">
                ...
                <span>
                    ...
                    <span class="block text-xs text-slate-500 mt-0.5">
                        {{ __('Turn this on to publish the online storefront on the company website — product catalog, cart, checkout and order tracking will appear automatically. Leave it off to keep the website as an information-only company page.') }}
                    </span>
                </span>
            </label>
