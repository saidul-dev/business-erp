<x-app-layout>
    <x-slot name="title">{{ __('Receive') }} — {{ $purchase->purchase_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Receive Items') }} — {{ $purchase->purchase_no }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Check off what actually arrived — leave a line at 0 to receive it later. Quantity, cost and inventory value are posted immediately for whatever you enter here.') }}</p>
        </div>
    </x-slot>
            ...
            <table class="w-full min-w-[860px] text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                        ...
                        <th class="px-5 py-3 font-semibold w-32">{{ __('Receive Now') }}</th>
