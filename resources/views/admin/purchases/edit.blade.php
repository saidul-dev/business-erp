<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Edit Purchase') }} — {{ $purchase->purchase_no }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Only possible while nothing has been received yet.') }}</p>
        </div>
    </x-slot>

    <div x-data="purchaseCart(@js(['itemOptions' => $itemOptions, 'items' => $items, 'discount' => (float) $purchase->discount_amount]))">
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 mb-4">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="mt-1">
                            <x-searchable-select name="party_id" :options="$suppliers" :selected="old('party_id', $purchase->party_id)"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <x-searchable-select name="site_id" :options="$sites" :selected="old('site_id', $purchase->site_id)"
                    ...
                    <div>
                        ...
                        <x-text-input id="order_date" name="order_date" type="date" class="mt-1 block w-full"
                                      :value="old('order_date', $purchase->order_date->toDateString())" required />
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-5 py-3">
                                    <input type="number" step="0.01" min="0"
                                           :name="'items[' + i + '][unit_cost]'" x-model.number="item.unit_cost"
                                           class="w-28 rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500" placeholder="0.00" required>
                                </td>
                                <td class="px-5 py-3 text-right font-semibold text-slate-800" x-text="((item.quantity || 0) * (item.unit_cost || 0)).toFixed(2)"></td>
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200" x-show="items.length" x-cloak>
                <div class="mb-5 max-w-xl">
                    ...
                    <textarea id="note" name="note" rows="2"
                              class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500">{{ old('note', $purchase->note) }}</textarea>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('purchases.show', $purchase) }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700">{{ __('Cancel') }}</a>
                </div>
            </div>
        </form>
    </div>
