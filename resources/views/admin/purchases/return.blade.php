<x-app-layout>
    <x-slot name="title">{{ __('Return') }} — {{ $purchase->purchase_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Return Items') }} — {{ $purchase->purchase_no }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Send received goods back to the supplier — leave a line at 0 to skip it. Inventory and Accounts Payable are adjusted immediately for whatever you enter here.') }}</p>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('purchases.return.store', $purchase) }}" x-data="{ fillReturnable() { this.$refs.form.querySelectorAll('[data-returnable]').forEach(el => { el.value = el.dataset.returnable; }); } }" x-ref="form">
        @csrf
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('purchases.show', $purchase) }}" class="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50">{{ __('Cancel') }}</a>
            </div>
        </div>
    </form>
</x-app-layout>
