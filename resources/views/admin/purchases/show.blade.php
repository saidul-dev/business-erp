<x-app-layout>
    ...
    @php
        $statusAccent = [
            ...
            'received' => 'border-emerald-400',
            'cancelled' => 'border-rose-400',
        ][$purchase->status] ?? 'border-slate-300';
        $due = $purchase->party->payableBalance();
    @endphp
    ...
    <div class="rounded-2xl border-l-4 {{ $statusAccent }} bg-white shadow-sm ring-1 ring-slate-200 p-6 mb-4">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-3 text-sm">
                ...
                <x-purchase-status-badge :status="$purchase->status" />
            </div>
        ...
        <div class="mt-5 grid grid-cols-1 sm:grid-cols-4 gap-3">
            <div class="rounded-xl bg-slate-50 p-3">
                ...
                <span class="mt-1.5 block text-sm font-semibold text-slate-700">{{ $purchase->order_date->format('d M, Y') }}</span>
            </div>
            <div class="rounded-xl bg-slate-50 p-3">
                <div class="flex items-center gap-2 text-slate-400">
                ...
                <span class="mt-1.5 block text-sm font-semibold text-slate-700">{{ $purchase->creator->name ?? '—' }}</span>
            </div>
            <div class="rounded-xl bg-brand-50/70 ring-1 ring-brand-100 p-3">
                ...
                <span class="mt-1.5 block text-xl font-bold text-brand-900">{{ number_format($purchase->total_amount, 2) }}</span>
                @if ($purchase->discount_amount > 0)
                <span class="block text-xs text-brand-400">{{ __('Discount') }}: {{ number_format($purchase->discount_amount, 2) }}</span>
                @endif
            </div>
            <div class="rounded-xl p-3 {{ $due > 0 ? 'bg-amber-50 ring-1 ring-amber-200' : 'bg-slate-50' }}">
                <div class="flex items-center gap-2 {{ $due > 0 ? 'text-amber-500' : 'text-slate-400' }}">
                    ...
                    <span class="text-[11px] font-semibold uppercase tracking-wide">{{ __('Supplier Due') }}</span>
                </div>
        ...
        <p class="mt-3 flex items-start gap-1.5 text-sm text-slate-500">
            ...
            <span class="italic">{{ $purchase->note }}</span>
        </p>
        @endif
        ...
        <table class="w-full min-w-[760px] text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-5 py-3 text-right tabular-nums text-slate-500">{{ rtrim(rtrim(number_format($item->received_quantity, 4), '0'), '.') ?: '0' }}</td>
                    ...
                    <td class="px-5 py-3 text-right tabular-nums text-slate-500">{{ number_format($item->unit_cost, 2) }}</td>
    ...
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden">
        ...
        <div class="flex flex-col items-center gap-2 px-5 py-10 text-center">
            ...
            <p class="text-sm text-slate-400">{{ __('Nothing received yet.') }}</p>
            <p class="text-xs text-slate-300">{{ __('Use "Receive Items" above once the shipment arrives.') }}</p>
        </div>
        @endforelse
    </div>
