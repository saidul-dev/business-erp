<x-app-layout>
    <x-slot name="title">{{ __('New Purchase') }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('New Purchase') }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Order stock from a supplier — nothing is added to inventory until it\'s received.') }}</p>
        </div>
    </x-slot>
