<x-app-layout>
    <x-slot name="title">{{ __('Purchases') }}</x-slot>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-brand-900">{{ __('Purchases') }}</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ __('Purchase orders and goods receipts from suppliers') }}</p>
            </div>
            @can('sourcing.create')
        ...
        <table class="w-full min-w-[860px] text-sm">
            <thead>
                <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                    <th class="px-5 py-3 font-semibold">{{ __('Purchase') }}</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-5 py-3 font-semibold text-slate-800">{{ $purchase->purchase_no }}</td>
                    <td class="px-5 py-3 text-slate-600">{{ $purchase->party->name }}</td>
                    <td class="px-5 py-3 text-slate-600">{{ $purchase->site->name }}</td>
                    <td class="px-5 py-3 text-slate-600">{{ $purchase->order_date->format('d M, Y') }}</td>
                    <td class="px-5 py-3 text-right font-semibold text-slate-800">{{ number_format($purchase->total_amount, 2) }}</td>
                    <td class="px-5 py-3">
                        <x-purchase-status-badge :status="$purchase->status" />
                    </td>
                    <td class="px-5 py-3 text-right">
                        <a href="{{ route('purchases.show', $purchase) }}" class="font-semibold text-accent-600 hover:text-accent-800">{{ __('View') }}</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="px-5 py-10 text-center text-slate-400">{{ __('No purchases yet.') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
