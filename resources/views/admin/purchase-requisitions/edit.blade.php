<x-app-layout>
    <x-slot name="title">{{ __('Edit Requisition') }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Edit Requisition') }} — {{ $purchaseRequisition->requisition_no }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Only possible while still pending.') }}</p>
        </div>
    </x-slot>

    <div x-data="requisitionCart(@js(['itemOptions' => $itemOptions, 'items' => $items]))">
        <form method="POST" action="{{ route('purchase-requisitions.update', $purchaseRequisition) }}" x-ref="form">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 mb-4">
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    <div>
                        ...
                        <div class="mt-1">
                            <x-searchable-select name="site_id" :options="$sites" :selected="old('site_id', $purchaseRequisition->site_id)"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <x-searchable-select name="party_id" :options="$suppliers" :selected="old('party_id', $purchaseRequisition->party_id)"
                    ...
                    <div>
                        ...
                        <x-text-input id="request_date" name="request_date" type="date" class="mt-1 block w-full"
                                      :value="old('request_date', $purchaseRequisition->request_date->toDateString())" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="required_by_date" name="required_by_date" type="date" class="mt-1 block w-full"
                                      :value="old('required_by_date', $purchaseRequisition->required_by_date?->toDateString())" />
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200" x-show="items.length" x-cloak>
                <div class="mb-5 max-w-xl">
                    ...
                    <textarea id="note" name="note" rows="2"
                              class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500">{{ old('note', $purchaseRequisition->note) }}</textarea>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('purchase-requisitions.show', $purchaseRequisition) }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700">{{ __('Cancel') }}</a>
                </div>
            </div>
        </form>
    </div>
