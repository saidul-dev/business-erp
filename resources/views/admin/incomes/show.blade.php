@php
    $amountInWords = \App\Support\AmountInWords::convert((float) $income->amount, \App\Models\CompanySetting::current()->currency_label);
@endphp
<x-app-layout>
    <x-slot name="title">{{ $income->income_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ $income->income_no }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Income detail') }}</p>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
        ...
        <div class="order-2 lg:order-1 space-y-6">
                ...
                <div class="p-6">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div>
                            ...
                            <span class="block text-lg font-semibold text-slate-800">{{ $income->category->name }}</span>
                        </div>
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-700 ring-1 ring-teal-200">
                    ...
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-5 text-sm">
                        <div>
                            ...
                            <span class="block font-semibold text-slate-700">{{ $income->receivedInto->name }}</span>
                        </div>
                        <div>
                            <span class="block text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Date') }}</span>
                            <span class="block text-slate-700">{{ $income->income_date->format('d M, Y') }}</span>
                        </div>
                        <div>
                            <span class="block text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Recorded By') }}</span>
                            <span class="block text-slate-700">{{ $income->creator->name ?? '—' }}</span>
                        </div>
                        ...
                        <div>
                            ...
                            <span class="block text-slate-700">{{ $income->reference_no }}</span>
                        </div>
                        @endif
                    </div>
            ...
            @if ($income->ledgerTransaction)
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <h3 class="font-bold text-brand-900">{{ __('Ledger Entry') }}</h3>
                    <span class="text-xs font-semibold text-slate-400">{{ $income->ledgerTransaction->voucher_no }}</span>
                </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($income->ledgerTransaction->lines as $line)
                        <tr>
        ...
        <div class="order-1 lg:order-2 lg:sticky lg:top-6 space-y-4">
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="p-5">
                    <span class="text-[11px] font-semibold uppercase tracking-wide text-teal-700">{{ __('Amount') }}</span>
                    <div class="mt-1 font-mono text-3xl font-black tabular-nums text-teal-700">{{ number_format($income->amount, 2) }}</div>
                    <p class="mt-3 border-t border-slate-100 pt-3 text-xs italic text-slate-500">{{ $amountInWords }}</p>
                </div>
            </div>

            <div class="flex flex-col gap-2">
                <a href="{{ route('incomes.print', $income) }}" target="_blank"
