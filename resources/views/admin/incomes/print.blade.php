@php
    $amountInWords = \App\Support\AmountInWords::convert((float) $income->amount, $company->currency_label);
@endphp
<x-app-layout>
    <x-slot name="title">{{ __('Print') }} — {{ $income->income_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Income Voucher') }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ $income->income_no }}</p>
        </div>
    </x-slot>
    ...
    <div class="print-area voucher-print mx-auto max-w-3xl rounded-2xl bg-white p-8 ring-1 ring-slate-200">
        ...
        <div class="flex items-start justify-between gap-6 border-b-2 border-brand-800 pb-6">
            ...
            <div class="text-right">
                <h2 class="text-lg font-bold uppercase tracking-wide text-brand-800">{{ __('Income Voucher') }}</h2>
                <p class="text-sm font-semibold text-slate-800">{{ $income->income_no }}</p>
                <p class="text-xs text-slate-500">{{ $income->income_date->format('d M, Y') }}</p>
            </div>
        </div>

        <div class="mt-6 grid grid-cols-2 gap-6 text-sm">
            <div>
                <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Received For') }}</p>
                <p class="font-semibold text-slate-800">{{ $income->category->name }}</p>
            </div>
            <div class="text-right">
                <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Received Into') }}</p>
                <p class="font-semibold text-slate-800">{{ $income->receivedInto->name }}</p>
                @if ($income->reference_no)
                <p class="text-slate-500">{{ __('Ref.') }}: {{ $income->reference_no }}</p>
                @endif
            </div>
        </div>

        <div class="mt-6 rounded-xl border-2 border-teal-200 bg-teal-50 px-6 py-5">
            <div class="flex items-baseline justify-between gap-4">
                <span class="text-[11px] font-semibold uppercase tracking-wide text-teal-800">{{ __('Amount') }}</span>
                <span class="font-mono text-3xl font-black tabular-nums text-teal-800">{{ number_format($income->amount, 2) }}</span>
            </div>
            <p class="mt-2 border-t border-teal-200 pt-2 text-xs italic text-slate-600">
        ...
        <div class="mt-6 text-sm">
            ...
            <p class="text-slate-700">{{ $income->note }}</p>
        </div>
        @endif
        ...
        <div class="mt-16 print-footer">
            <div class="grid grid-cols-2 gap-6 text-sm">
                <div class="border-t border-slate-400 pt-2 text-slate-600">
                    {{ __('Prepared By') }} ({{ $income->creator->name ?? '—' }})
                </div>
