<x-app-layout>
    ...
    @elseif ($products->isEmpty() && $variantGroups->isEmpty())
        <div class="rounded-2xl bg-white p-10 text-center text-slate-400 shadow-sm ring-1 ring-slate-200">
    ...
    @else
        <form method="POST" action="{{ route('stock.initial.store') }}">
            ...
            <h3 class="mb-2 text-sm font-semibold text-slate-600">{{ __('Products') }}</h3>
            ...
            <h3 class="mb-2 text-sm font-semibold text-slate-600">{{ __('Product Variants') }}</h3>
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden mb-6">
                <div class="overflow-x-auto">
                <table class="w-full min-w-[900px] text-sm">
                    <thead>
