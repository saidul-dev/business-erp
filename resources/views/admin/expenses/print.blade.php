@php
    $amountInWords = \App\Support\AmountInWords::convert((float) $expense->amount, $company->currency_label);
@endphp
<x-app-layout>
    <x-slot name="title">{{ __('Print') }} — {{ $expense->expense_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Expense Voucher') }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ $expense->expense_no }}</p>
        </div>
    </x-slot>
    ...
    <div class="print-area voucher-print mx-auto max-w-3xl rounded-2xl bg-white p-8 ring-1 ring-slate-200">
        ...
        <div class="flex items-start justify-between gap-6 border-b-2 border-brand-800 pb-6">
            ...
            <div class="text-right">
                <h2 class="text-lg font-bold uppercase tracking-wide text-brand-800">{{ __('Expense Voucher') }}</h2>
                <p class="text-sm font-semibold text-slate-800">{{ $expense->expense_no }}</p>
        ...
        <div class="mt-6 grid grid-cols-2 gap-6 text-sm">
            <div>
                <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Paid For') }}</p>
                <p class="font-semibold text-slate-800">{{ $expense->category->name }}</p>
            </div>
            <div class="text-right">
                <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Paid From') }}</p>
                <p class="font-semibold text-slate-800">{{ $expense->paidFrom->name }}</p>
                @if ($expense->reference_no)
                <p class="text-slate-500">{{ __('Ref.') }}: {{ $expense->reference_no }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="mt-6 rounded-xl border-2 border-amber-200 bg-amber-50 px-6 py-5">
            <div class="flex items-baseline justify-between gap-4">
                <span class="text-[11px] font-semibold uppercase tracking-wide text-amber-800">{{ __('Amount') }}</span>
                <span class="font-mono text-3xl font-black tabular-nums text-amber-800">{{ number_format($expense->amount, 2) }}</span>
            </div>
        ...
        <div class="mt-6 text-sm">
            ...
            <p class="text-slate-700">{{ $expense->note }}</p>
        </div>
        @endif
        ...
        <div class="mt-16 print-footer">
            <div class="grid grid-cols-2 gap-6 text-sm">
                <div class="border-t border-slate-400 pt-2 text-slate-600">
                    {{ __('Prepared By') }} ({{ $expense->creator->name ?? '—' }})
                </div>
