<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-brand-900">{{ __('Users') }}</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ __('Team members and their assigned roles') }}</p>
            </div>
            @can('users.create')
        ...
        <table class="w-full min-w-[640px] text-sm">
            <thead>
                <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                    ...
                    <th class="px-5 py-3 font-semibold">{{ __('Role(s)') }}</th>
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-end gap-1">
                            ...
                            <form method="POST" action="{{ route('users.destroy', $user) }}"
                                  onsubmit="return confirm('{{ __('Delete user :name? This cannot be undone.', ['name' => $user->name]) }}');">
                                @csrf
                                @method('DELETE')
