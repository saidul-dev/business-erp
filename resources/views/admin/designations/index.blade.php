<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-brand-900">{{ __('Designations') }}</h2>
        ...
        <table class="w-full min-w-[600px] text-sm">
            <thead>
                <tr class="border-b border-slate-100 bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                    <th class="px-5 py-3 font-semibold">{{ __('Designation') }}</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-5 py-3 font-semibold text-slate-800">{{ $designation->name }}</td>
                    <td class="px-5 py-3 text-slate-600">{{ $designation->employees_count }}</td>
                    <td class="px-5 py-3">
                        @can('hrm.edit')
                        <form method="POST" action="{{ route('designations.toggle-status', $designation) }}">
                            ...
                            <button type="submit"
                                    ...
                                    {{ $designation->status ? 'bg-emerald-50 text-emerald-600 ring-emerald-200' : 'bg-slate-100 text-slate-500 ring-slate-200' }}">
                                {{ $designation->status ? __('Active') : __('Inactive') }}
                            </button>
                        </form>
                        @else
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-end gap-1">
                            @can('hrm.edit')
                            <a href="{{ route('designations.edit', $designation) }}" title="{{ __('Edit') }}"
                            ...
                            <form method="POST" action="{{ route('designations.destroy', $designation) }}"
                                  onsubmit="return confirm('{{ __('Delete designation :name? This cannot be undone.', ['name' => $designation->name]) }}');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-5 py-10 text-center text-slate-400">{{ __('No designations yet — add your first designation.') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
