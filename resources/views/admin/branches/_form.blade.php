...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                              :value="old('name', $branch->name ?? '')" required autofocus placeholder="{{ __('e.g. Dhaka Central Warehouse') }}" />
            ...
            <div>
                ...
                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full uppercase"
                              :value="old('code', $branch->code ?? '')" required placeholder="{{ __('e.g. DHK-BR') }}" />
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full"
                              :value="old('phone', $branch->phone ?? '')" placeholder="{{ __('e.g. 01700-000000') }}" />
            ...
            <div>
                ...
                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full"
                              :value="old('email', $branch->email ?? '')" placeholder="{{ __('branch@company.com') }}" />
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 h-fit">
        <div class="mb-4 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-brand-900">{{ __('Branch Type') }}</h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2.5">
            ...
            <label class="cursor-pointer">
                <input type="radio" name="type" value="{{ $type }}" class="peer sr-only"
                       @checked(old('type', $branch->type ?? '') === $type) required>
    ...
    <div class="lg:col-span-3 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <textarea id="address" name="address" rows="3"
                  ...
                  placeholder="{{ __('Street, area, city, postal code') }}">{{ old('address', $branch->address ?? '') }}</textarea>
        <x-input-error class="mt-2" :messages="$errors->get('address')" />
    </div>
</div>

<div class="mt-5 flex items-center gap-3">
    <x-primary-button>{{ $editing ? __('Update Branch') : __('Create Branch') }}</x-primary-button>
    <a href="{{ route('branches.index') }}" class="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50">{{ __('Cancel') }}</a>
</div>
