<x-app-layout>
    <x-slot name="title">{{ __('Branches') }}</x-slot>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-brand-900">{{ __('Branches') }}</h2>
                <p class="text-sm text-slate-500 mt-0.5">{{ __('Branches, warehouses, outlets and other business locations') }}</p>
            </div>
            @can('branches.create')
            <a href="{{ route('branches.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-800 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 focus:ring-2 focus:ring-accent-500 focus:ring-offset-2">
        ...
        <table class="w-full min-w-[720px] text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    <td class="px-5 py-3">
                        <div>
                            <span class="block font-semibold text-slate-800">{{ $branch->name }}</span>
                            @if ($branch->address)
                                <span class="block text-xs text-slate-400">{{ Str::limit($branch->address, 50) }}</span>
                            @endif
                        </div>
                    </td>
                    <td class="px-5 py-3 text-slate-600">{{ $branch->code }}</td>
                    <td class="px-5 py-3">
                        <span class="inline-flex rounded-full bg-brand-50 px-2.5 py-0.5 text-xs font-semibold text-brand-700 ring-1 ring-brand-200">{{ $branch->type }}</span>
                    </td>
                    <td class="px-5 py-3">
                    ...
                    <td class="px-5 py-3">
                        ...
                        <form method="POST" action="{{ route('branches.toggle-status', $branch) }}">
                            ...
                            <button type="submit"
                                    ...
                                    {{ $branch->status ? 'bg-emerald-50 text-emerald-600 ring-emerald-200' : 'bg-slate-100 text-slate-500 ring-slate-200' }}">
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center justify-end gap-1">
                            @can('branches.edit')
                            <a href="{{ route('branches.edit', $branch) }}" title="{{ __('Edit') }}"
                            ...
                            <form method="POST" action="{{ route('branches.destroy', $branch) }}"
                                  onsubmit="return confirm('{{ __('Delete branch :name? This cannot be undone.', ['name' => $branch->name]) }}');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-5 py-10 text-center text-slate-400">{{ __('No branches yet — add your first branch, warehouse or outlet.') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
