...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4" x-data="{ preview: @js($brand->logo_url ?? null), removeLogo: false }">
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div class="mb-4">
            <h3 class="font-bold text-brand-900">Brand Logo</h3>
    ...
    <div class="lg:col-span-2 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 space-y-5">
        <div>
            <h3 class="font-bold text-brand-900">Brand Details</h3>
            <p class="text-xs text-slate-400">Shown on products and reports</p>
        </div>
...
<div class="mt-5 flex items-center gap-3">
    <x-primary-button>{{ $editing ? 'Update Brand' : 'Create Brand' }}</x-primary-button>
    <a href="{{ route('brands.index') }}" class="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 ring-1 ring-slate-200 hover:bg-slate-50">Cancel</a>
</div>
