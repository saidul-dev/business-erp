<x-app-layout>
    <x-slot name="title">{{ __('Print') }} — {{ $saleReturn->return_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Sales Return Note') }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ $saleReturn->return_no }}</p>
        </div>
    </x-slot>
    ...
    <div class="print-area voucher-print mx-auto max-w-3xl rounded-2xl bg-white p-8 ring-1 ring-slate-200">
        ...
        <div class="flex items-start justify-between gap-6 border-b-2 border-brand-800 pb-6">
            ...
            <div class="text-right">
                <h2 class="text-lg font-bold uppercase tracking-wide text-brand-800">{{ __('Sales Return Note') }}</h2>
                <p class="text-sm font-semibold text-slate-800">{{ $saleReturn->return_no }}</p>
                <p class="text-xs text-slate-500">{{ $saleReturn->return_date->format('d M, Y') }}</p>
            </div>
        </div>
        ...
        <div class="mt-6 grid grid-cols-2 gap-6 text-sm">
            <div>
                ...
                <p class="font-semibold text-slate-800">{{ $saleReturn->sale->party->name }}</p>
                @if ($saleReturn->sale->party->phone)<p class="text-slate-600">{{ $saleReturn->sale->party->phone }}</p>@endif
                @if ($saleReturn->sale->party->address)<p class="text-slate-500">{{ $saleReturn->sale->party->address }}</p>@endif
            </div>
            <div class="text-right">
                <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Returned To') }}</p>
                <p class="font-semibold text-slate-800">{{ $saleReturn->sale->site->name }}</p>
                <p class="text-slate-500">{{ __('Against SO') }}: {{ $saleReturn->sale->sale_no }}</p>
            </div>
        </div>
        ...
        <table class="mt-6 w-full text-sm">
            ...
            <tbody>
                ...
                @foreach ($saleReturn->items as $i => $ri)
                @php $lineValue = $ri->quantity * $ri->saleItem->unit_price; $totalValue += $lineValue; @endphp
                <tr class="border-b border-slate-200">
                    <td class="py-2 pr-2 align-top">{{ $i + 1 }}</td>
                    <td class="py-2 pr-2 align-top">
                        {{ $ri->saleItem->product->name }}{{ $ri->saleItem->productVariant ? ' — '.$ri->saleItem->productVariant->label : '' }}
                    ...
                    <td class="py-2 pr-2 text-right align-top">
                        {{ rtrim(rtrim(number_format($ri->quantity, 4), '0'), '.') ?: '0' }} {{ $ri->saleItem->product->stockUnit?->short_name }}
                    </td>
                    <td class="py-2 pr-2 text-right align-top">{{ number_format($ri->saleItem->unit_price, 2) }}</td>
        ...
        <div class="mt-6 text-sm">
            ...
            <p class="text-slate-700">{{ $saleReturn->note }}</p>
        </div>
        @endif
        ...
        <div class="mt-16 print-footer">
            <div class="grid grid-cols-2 gap-6 text-sm">
                <div class="border-t border-slate-400 pt-2 text-slate-600">
                    {{ __('Returned By') }} ({{ $saleReturn->returnedBy->name ?? '—' }})
                </div>
