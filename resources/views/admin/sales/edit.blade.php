<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Edit Sale') }} — {{ $sale->sale_no }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ __('Only possible while nothing has been delivered yet.') }}</p>
        </div>
    </x-slot>

    <div x-data="saleCart(@js(['itemOptions' => $itemOptions, 'items' => $items, 'discount' => (float) $sale->discount_amount]))">
        <form method="POST" action="{{ route('sales.update', $sale) }}" x-ref="form">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 mb-4">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="mt-1">
                            <x-searchable-select name="party_id" :options="$customers" :selected="old('party_id', $sale->party_id)"
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <x-searchable-select name="site_id" :options="$sites" :selected="old('site_id', $sale->site_id)"
                    ...
                    <div>
                        ...
                        <x-text-input id="order_date" name="order_date" type="date" class="mt-1 block w-full"
                                      :value="old('order_date', $sale->order_date->toDateString())" required />
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200" x-show="items.length" x-cloak>
                <div class="mb-5 max-w-xl">
                    ...
                    <textarea id="note" name="note" rows="2"
                              class="mt-1 block w-full rounded-lg border-slate-300 text-sm focus:border-accent-500 focus:ring-accent-500">{{ old('note', $sale->note) }}</textarea>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('sales.show', $sale) }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700">{{ __('Cancel') }}</a>
                </div>
            </div>
        </form>
    </div>
