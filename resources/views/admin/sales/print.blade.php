<x-app-layout>
    <x-slot name="title">{{ __('Print') }} — {{ $sale->sale_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Sales Order') }}</h2>
    ...
    <div class="print-area mx-auto max-w-3xl rounded-2xl bg-white p-8 ring-1 ring-slate-200">
        ...
        <div class="flex items-start justify-between gap-6 border-b-2 border-brand-800 pb-6">
            ...
            <div class="text-right">
                <h2 class="text-lg font-bold uppercase tracking-wide text-brand-800">{{ __('Sales Order') }}</h2>
                <p class="text-sm font-semibold text-slate-800">{{ $sale->sale_no }}</p>
                <p class="text-xs text-slate-500">{{ $sale->order_date->format('d M, Y') }}</p>
            </div>
        </div>
        ...
        <div class="mt-6 grid grid-cols-2 gap-6 text-sm">
            ...
            <div class="text-right">
                ...
                <p class="font-semibold text-slate-800">{{ $sale->site->name }}</p>
                <p class="text-slate-500">{{ __('Status') }}: {{ ucfirst($sale->status) }}</p>
            </div>
        </div>
        ...
        <div class="mt-3 text-right text-sm">
            <div class="text-slate-500">{{ __('Subtotal') }}: {{ number_format($sale->subtotal_amount, 2) }}</div>
            @if ($sale->discount_amount > 0)
            <div class="text-slate-500">{{ __('Discount') }}: {{ number_format($sale->discount_amount, 2) }}</div>
            @endif
            <div class="font-bold text-slate-800">{{ __('Total') }}: {{ number_format($sale->total_amount, 2) }}</div>
        </div>

        @if ($sale->note)
        <div class="mt-6 text-sm">
            <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Note') }}</p>
            <p class="text-slate-700">{{ $sale->note }}</p>
        </div>
        @endif
        ...
        <div class="mt-16 print-footer">
            <div class="grid grid-cols-2 gap-6 text-sm">
                <div class="border-t border-slate-400 pt-2 text-slate-600">
                    {{ __('Prepared By') }} ({{ $sale->creator->name ?? '—' }})
                </div>
