<x-app-layout>
    <x-slot name="title">{{ __('Print') }} — {{ $delivery->delivery_no }}</x-slot>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-brand-900">{{ __('Delivery Note') }}</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ $delivery->delivery_no }}</p>
        </div>
    </x-slot>

    <div class="no-print mb-4 flex justify-end gap-3">
        <a href="{{ route('sales.show', $delivery->sale) }}"
    ...
    <div class="print-area mx-auto max-w-3xl rounded-2xl bg-white p-8 ring-1 ring-slate-200">
        ...
        <div class="flex items-start justify-between gap-6 border-b-2 border-brand-800 pb-6">
            ...
            <div class="text-right">
                <h2 class="text-lg font-bold uppercase tracking-wide text-brand-800">{{ __('Delivery Note') }}</h2>
                <p class="text-sm font-semibold text-slate-800">{{ $delivery->delivery_no }}</p>
                <p class="text-xs text-slate-500">{{ $delivery->delivered_date->format('d M, Y') }}</p>
            </div>
        </div>
        ...
        <div class="mt-6 grid grid-cols-2 gap-6 text-sm">
            <div>
                ...
                <p class="font-semibold text-slate-800">{{ $delivery->sale->party->name }}</p>
                @if ($delivery->sale->party->phone)<p class="text-slate-600">{{ $delivery->sale->party->phone }}</p>@endif
                @if ($delivery->sale->party->address)<p class="text-slate-500">{{ $delivery->sale->party->address }}</p>@endif
            </div>
            <div class="text-right">
                <p class="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Shipped From') }}</p>
                <p class="font-semibold text-slate-800">{{ $delivery->sale->site->name }}</p>
                <p class="text-slate-500">{{ __('Against SO') }}: {{ $delivery->sale->sale_no }}</p>
            </div>
        </div>
        ...
        <table class="mt-6 w-full text-sm">
            <thead>
                <tr class="border-b-2 border-brand-800 text-left">
                    ...
                    <th class="py-2 pr-2 text-right">{{ __('Qty Delivered') }}</th>
                    <th class="py-2">{{ __('Batch / Expiry / Serial') }}</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($delivery->items as $i => $di)
                <tr class="border-b border-slate-200">
                    <td class="py-2 pr-2 align-top">{{ $i + 1 }}</td>
                    <td class="py-2 pr-2 align-top">
                        {{ $di->saleItem->product->name }}{{ $di->saleItem->productVariant ? ' — '.$di->saleItem->productVariant->label : '' }}
                    </td>
                    <td class="py-2 pr-2 text-right align-top">
                        {{ rtrim(rtrim(number_format($di->quantity, 4), '0'), '.') ?: '0' }} {{ $di->saleItem->product->stockUnit?->short_name }}
                    </td>
                    <td class="py-2 align-top text-slate-500">
                        {{ collect([$di->batch_no, $di->expiry_date?->format('d M, Y'), $di->serial_no])->filter()->implode(' · ') ?: '—' }}
                    </td>
                </tr>
                @endforeach
            </tbody>
        ...
        <div class="mt-6 text-sm">
            ...
            <p class="text-slate-700">{{ $delivery->note }}</p>
        </div>
        @endif
        ...
        <div class="mt-16 print-footer">
            <div class="grid grid-cols-2 gap-6 text-sm">
                <div class="border-t border-slate-400 pt-2 text-slate-600">
                    {{ __('Delivered By') }} ({{ $delivery->deliveredBy->name ?? '—' }})
                </div>
