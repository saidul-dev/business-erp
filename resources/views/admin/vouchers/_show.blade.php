@php
    ...
    $initials = collect(explode(' ', trim($party->name)))
        ...
        ->implode('');
    $amountInWords = \App\Support\AmountInWords::convert((float) $amount, $currencyLabel ?? 'Taka');
@endphp
...
<div class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
    ...
    <div class="order-2 lg:order-1 space-y-6">
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                ...
                <span class="text-xs font-semibold text-slate-400">{{ $ledgerTransaction->voucher_no }}</span>
            </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($ledgerTransaction->lines as $line)
                    <tr>
    ...
    <div class="order-1 lg:order-2 lg:sticky lg:top-6 space-y-4">
        <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div class="border-t border-slate-100 p-5" style="background:{{ $palette['soft'] }}">
                <span class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{{ $balanceLabel }}</span>
                <div class="mt-1 text-xl font-bold text-slate-800">{{ number_format($currentBalance, 2) }}</div>
            </div>
        </div>

        <div class="flex flex-col gap-2">
            <a href="{{ $printUrl }}" target="_blank"
               class="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90"
               style="background:{{ $palette['solid'] }}">
