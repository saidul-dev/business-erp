<x-website-layout :title="__('Career')">
    ...
    <section class="bg-gradient-to-br from-brand-950 via-brand-900 to-brand-800 py-20 text-center">
        <div class="mx-auto max-w-3xl px-4">
            ...
            <h1 class="mt-6 text-4xl font-extrabold text-white">{{ __("We're Growing — Join Us") }}</h1>
            <p class="mt-4 max-w-xl mx-auto text-brand-200/90">
                {{ __("We're always looking for good people. Send us your CV and we'll reach out when a role fits.") }}
            </p>
        </div>
    </section>
    ...
    <section class="bg-white py-20 sm:py-28">
        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <h2 class="mt-6 text-2xl font-bold text-brand-950">{{ __('No open positions listed right now') }}</h2>
            <p class="mt-3 text-slate-600">
                {{ __("That doesn't mean we're not hiring. Email your CV and a short note about what you're looking for, and we'll keep it on file.") }}
            </p>
            @if ($company->email)
            ...
            @else
            <p class="mt-8 text-sm text-slate-400">{{ __('Contact details will appear here once added in Admin > Settings.') }}</p>
            @endif
        </div>
    </section>
