<x-website-layout :title="'Track Order'">

    <section class="bg-white py-16">
        <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
            <div class="text-center">
                <h1 class="text-3xl font-extrabold text-brand-950">Track Your Order</h1>
                <p class="mt-2 text-slate-500">Enter your order number and the phone number you checked out with.</p>
            </div>
            ...
            <form method="POST" action="{{ route('track-order.result') }}" class="mt-8 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 space-y-4">
                @csrf
                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">Order Number</label>
                ...
                <div class="mt-5 border-t border-slate-100 pt-4">
                    ...
                    <div class="text-sm text-slate-600">
                        ...
                        @if ($delivery->consignment)
                            via {{ $delivery->consignment->deliveryPartner->name }}
                            @if ($delivery->consignment->tracking_no)
                                (Tracking: {{ $delivery->consignment->tracking_no }})
                            @endif
                        @endif
                    </div>
                    @endforeach
