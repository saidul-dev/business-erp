<x-website-layout :title="__('Checkout')">

    <section class="bg-white py-12">
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <h1 class="text-2xl font-extrabold text-brand-950">{{ __('Checkout') }}</h1>
            ...
            <div class="mt-5 flex items-center gap-3 rounded-xl bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700 ring-1 ring-amber-200">
                <span>{{ __("The online store isn't fully set up yet — please contact us directly to place your order.") }}</span>
            </div>
            @endif
            ...
            <div class="mt-6 grid gap-8 lg:grid-cols-[1fr_360px]"
                ...
                <form method="POST" action="{{ route('checkout.store') }}" @submit="submitting = true"
                    ...
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">{{ __('Full Name') }}</label>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-slate-400">{{ __('Used to track your order later — no account needed.') }}</p>
                    </div>

                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">{{ __('Delivery Address') }}</label>
                    ...
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">{{ __('Delivery Area') }}</label>
                        ...
                        <p class="mt-1 text-xs text-slate-400">{{ __('Delivery charge is collected by the courier on delivery — not included in the order total below.') }}</p>
                    </div>
                    @endif

                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">{{ __('Order Note (optional)') }}</label>
                        <textarea name="note" rows="2"
                                  class="w-full rounded-lg border-slate-300 focus:border-accent-500 focus:ring-accent-500">{{ old('note') }}</textarea>
                    </div>

                    <div>
                        <p class="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">{{ __('Payment Method') }}</p>
                        <div class="space-y-2">
                            ...
                            <label class="flex items-center gap-2 rounded-lg bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700 ring-1 ring-transparent has-[:checked]:ring-accent-500">
                                ...
                                {{ __('Online Payment (bKash, Nagad, Card, Rocket)') }}
                            </label>
                        </div>
                    ...
                    <button type="submit" :disabled="submitting || {{ $company->online_site_id ? 'false' : 'true' }}"
                        ...
                        <span x-show="!submitting" x-text="paymentMethod === 'sslcommerz' ? '{{ __('Continue to Payment') }}' : '{{ __('Place Order') }}'"></span>
                        <span x-show="submitting" x-cloak>{{ __('Placing Order…') }}</span>
                    </button>
                </form>

                <div class="h-fit rounded-2xl bg-slate-50 p-6 ring-1 ring-slate-200">
                    <p class="mb-4 font-semibold text-brand-950">{{ __('Order Summary') }}</p>
                    <div class="space-y-3">
                    ...
                    <div class="mt-4 flex items-center justify-between border-t border-slate-200 pt-4">
                        <span class="font-semibold text-slate-700" x-text="paymentMethod === 'sslcommerz' ? '{{ __('Total (Pay Online)') }}' : '{{ __('Total (Cash on Delivery)') }}'"></span>
                    ...
                    <template x-if="zone">
                        <div class="mt-3 flex items-center justify-between rounded-lg bg-amber-50 px-3 py-2 text-sm ring-1 ring-amber-200">
                            <span class="text-amber-700">{{ __('Delivery') }} (<span x-text="zone?.name"></span>) — {{ __('pay courier') }}</span>
