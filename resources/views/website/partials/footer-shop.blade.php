...
<footer class="bg-brand-950 text-brand-200/80 border-t border-white/10">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
            <p class="text-sm font-semibold text-white mb-3">{{ __('Shop') }}</p>
            <ul class="space-y-2 text-sm">
                <li><a href="{{ route('shop') }}" class="hover:text-white">{{ __('All Products') }}</a></li>
                <li><a href="{{ route('cart') }}" class="hover:text-white">{{ __('Cart') }}</a></li>
                <li><a href="{{ route('track-order') }}" class="hover:text-white">{{ __('Track Order') }}</a></li>
            </ul>
        </div>
    ...
    <div class="border-t border-white/10 py-6 text-center text-xs text-brand-300/70">
        &copy; {{ date('Y') }} {{ $company->name ?? 'Business ERP' }} &middot; {{ __('Online Store') }}
