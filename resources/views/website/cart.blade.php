<x-website-layout :title="__('Cart')">

    <section class="bg-white py-12">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <h1 class="text-2xl font-extrabold text-brand-950">{{ __('Your Cart') }}</h1>
            ...
            <div class="mt-8 rounded-2xl border border-dashed border-slate-200 py-20 text-center">
                <p class="font-semibold text-slate-500">{{ __('Your cart is empty.') }}</p>
                <a href="{{ route('shop') }}" class="mt-2 inline-block text-sm font-semibold text-accent-600 hover:text-accent-800">{{ __('Continue shopping') }}</a>
            </div>
            @else
            <div class="mt-6 rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden">
                <div class="divide-y divide-slate-100">
                    ...
                    <div class="flex items-center gap-4 p-5">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-sm text-slate-500">{{ number_format($line['unit_price'], 2) }} {{ __('each') }}</p>
                        </div>
                        ...
                        <form method="POST" action="{{ route('cart.update', $line['key']) }}" class="flex items-center gap-2">
                            ...
                            <button type="submit" class="text-xs font-semibold text-slate-500 hover:text-slate-700">{{ __('Update') }}</button>
                        </form>
                        ...
                        <form method="POST" action="{{ route('cart.remove', $line['key']) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-slate-400 hover:text-rose-600" title="{{ __('Remove') }}">
                ...
                <div class="flex items-center justify-between bg-slate-50 px-5 py-4">
                    <span class="font-semibold text-slate-600">{{ __('Subtotal') }}</span>
                    <span class="text-xl font-bold text-brand-950">{{ number_format($subtotal, 2) }}</span>
                </div>
            </div>

            <div class="mt-6 flex items-center justify-between">
                <a href="{{ route('shop') }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700">← {{ __('Continue shopping') }}</a>
