        ...
        <div class="min-h-screen">
            ...
            <aside class="fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-brand-900 via-brand-900 to-brand-950 text-brand-100 flex flex-col transform transition-all duration-200 lg:translate-x-0"
                ...
                <nav class="sidebar-scroll flex-1 overflow-y-auto overflow-x-hidden px-3 py-4 space-y-6">
                    <div>
                        <p class="nav-label px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-brand-300/70" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Main') }}</p>
                        <x-sidebar-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" :title="__('Dashboard')">
                            ...
                            <span class="nav-label" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Dashboard') }}</span>
                        </x-sidebar-link>
                    </div>

                    <div>
                        <p class="nav-label px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-brand-300/70" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Operations') }}</p>
                        ...
                        <x-sidebar-dropdown :title="__('Purchase')" :active="request()->routeIs('purchases.*')">
                            ...
                            <x-sidebar-sublink :href="route('purchases.manual')" :active="request()->routeIs('purchases.manual')">
                                {{ __('User Manual') }}
                            </x-sidebar-sublink>
                        </x-sidebar-dropdown>
                        @endcan
                        ...
                        @can('inventory.view')
                        <x-sidebar-dropdown :title="__('Inventory')" :active="request()->routeIs('products.*', 'categories.*', 'brands.*', 'units.*', 'attributes.*', 'stock.*')">
                            <x-slot name="icon">
                        ...
                        @can('parties.view')
                        <x-sidebar-link :href="route('parties.index')" :active="request()->routeIs('parties.*')" :title="__('Parties')">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z"/></svg>
                            <span class="nav-label" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Parties') }}</span>
                        </x-sidebar-link>
                        @endcan

                        @can('accounts.view')
                        <x-sidebar-dropdown :title="__('Accounts')" :active="request()->routeIs('bank-accounts.*', 'payments.*', 'collections.*', 'expenses.*', 'incomes.*', 'fund-transfers.*', 'capital-transactions.*', 'day-book.*', 'trial-balance.*', 'balance-sheet.*', 'profit-loss.*', 'due-report.*')">
                            <x-slot name="icon">
                                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/></svg>
                            </x-slot>

                            <x-sidebar-sublink :href="route('bank-accounts.index')" :active="request()->routeIs('bank-accounts.*')">
                                {{ __('Bank Accounts') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('payments.index')" :active="request()->routeIs('payments.*')">
                                {{ __('Payments') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('collections.index')" :active="request()->routeIs('collections.*')">
                                {{ __('Collections') }}
                            </x-sidebar-sublink>
                            ...
                            <x-sidebar-sublink :href="route('fund-transfers.index')" :active="request()->routeIs('fund-transfers.*')">
                                {{ __('Fund Transfer') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('capital-transactions.index')" :active="request()->routeIs('capital-transactions.*')">
                                {{ __('Investment') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('day-book.index')" :active="request()->routeIs('day-book.*')">
                                {{ __('Day Book') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('trial-balance.index')" :active="request()->routeIs('trial-balance.*')">
                                {{ __('Trial Balance') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('balance-sheet.index')" :active="request()->routeIs('balance-sheet.*')">
                                {{ __('Balance Sheet') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('profit-loss.index')" :active="request()->routeIs('profit-loss.*')">
                                {{ __('Profit & Loss') }}
                            </x-sidebar-sublink>
                            <x-sidebar-sublink :href="route('due-report.index')" :active="request()->routeIs('due-report.*')">
                                {{ __('Due Report') }}
                            </x-sidebar-sublink>
                        </x-sidebar-dropdown>
                        @endcan
                        ...
                        <x-sidebar-link href="#" :title="__('Tasks')">
                            ...
                            <span class="nav-label" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Tasks') }}</span>
                        </x-sidebar-link>
                        @endcan

                        @can('hrm.view')
                        <x-sidebar-link href="#" :title="__('HRM')">
                            ...
                            <span class="nav-label" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('HRM') }}</span>
                        </x-sidebar-link>
                        @endcan
                        ...
                        <x-sidebar-link href="#" :title="__('Reports')">
                            ...
                            <span class="nav-label" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Reports') }}</span>
                        </x-sidebar-link>
                        @endcan
                    </div>
                    ...
                    <div>
                        <p class="nav-label px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-brand-300/70" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Administration') }}</p>

                        @can('sites.view')
                        <x-sidebar-link :href="route('sites.index')" :active="request()->routeIs('sites.*')" :title="__('Sites')">
                            ...
                            <span class="nav-label" :class="$store.sidebar.collapsed && 'lg:hidden'">{{ __('Sites') }}</span>
                        </x-sidebar-link>
                        @endcan
                    ...
                    <div class="ml-auto flex items-center gap-1.5 sm:gap-3 shrink-0">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="hidden sm:flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-brand-800 ring-1 ring-slate-200 hover:bg-slate-50">
                                <svg class="h-4 w-4 shrink-0 text-accent-600" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0 0c-2.485 0-4.5-4.03-4.5-9s2.015-9 4.5-9 4.5 4.03 4.5 9-2.015 9-4.5 9Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M3.6 9h16.8M3.6 15h16.8"/></svg>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.outside="open = false" x-transition
                                 ...
                                 style="display: none;">
                                <p class="px-4 pt-1 pb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Current Site') }}</p>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-2 sm:gap-3 rounded-lg px-1.5 sm:px-2 py-1.5 hover:bg-slate-100">
                                ...
                                <span class="hidden md:block text-left">
                                    ...
                                    <span class="block text-[11px] font-medium text-accent-600">{{ Auth::user()->getRoleNames()->first() ?? __('No role') }}</span>
                                </span>
                            ...
                            <div x-show="open" @click.outside="open = false" x-transition
                                 ...
                                 style="display: none;">
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-brand-800">{{ __('Profile') }}</a>
